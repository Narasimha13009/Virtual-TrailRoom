...
def cvloop(run_event):
    ...
    while run_event.is_set():
        ...
        for face in faces:
            ...
            if SPRITES[3]:  # Tiara
                apply_sprite(
                    image, IMAGES[3][ACTIVE_IMAGES[3]], w+45, x-20, y+20, incl, ontop=True)

            # Necklaces
            if SPRITES[1]:
                (x1, y1, w1, h1) = get_face_boundbox(shape, 6)
                apply_sprite(
                    image, IMAGES[1][ACTIVE_IMAGES[1]], w1, x1, y1+125, incl,  ontop=False)

            # Goggles
            if SPRITES[6]:
                (x3, y3, _, h3) = get_face_boundbox(shape, 1)
                apply_sprite(
                    image, IMAGES[6][ACTIVE_IMAGES[6]], w, x, y3-5, incl, ontop=False)
...
center = Frame(root, bg='#cdcf5f', width=50, height=40, padx=3, pady=3)
...
SPRITES_new = [0 for i in range(10)]
BTNS_new = []
IMAGES_new = {i: [] for i in range(10)}
PHOTOS_new = {i: [] for i in range(10)}

for img in sys.argv[1:]:
    print("IMAGEEEEEEEEEEE======>>>>>>>>>>>>>", img)
    ...
    # print("IMAGEEEEEEEEEEE======>>>>>>>>>>>>>",img)
    img = "images/Necklace1/1.png"
    IMAGES_new[int(img.rsplit('/', 1)[0][-1])].append(img)
    image = ImageTk.PhotoImage(Image.open(img).resize((150, 100)))
    PHOTOS_new[int(img.rsplit('/', 1)[0][-1])].append(image)
    print("end")
    # IMAGES[int(img.rsplit('/',2)[0][-1])].append(img)
    # image=ImageTk.PhotoImage(Image.open(img).resize((150,100)))
    # PHOTOS[int(img.rsplit('/',2)[0][-1])].append(image)

for index in range(9):
    if len(PHOTOS[index]) > 0:
        for k, photo in enumerate(PHOTOS[index]):
            btn = Button(scrollable_body, highlightbackground='white', text=IMAGES[index][k].rsplit('/', 1)[1].replace('.png', '')[
                         :-1], bg='white', image=photo, command=lambda index=index, k=k: put_sprite(index, k), compound=LEFT, width='300', height='200')
