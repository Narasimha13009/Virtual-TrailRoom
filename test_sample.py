...
def apply_sprite2feature(image, sprite_path, haar_filter, x_offset, y_offset, y_offset_image, adjust2feature, desired_width, x, y, w, h):
    ...
    if len(feature) != 0:
        # adjust only to feature in y axis (eyes)
        ...
        if adjust2feature:
            ...
            ypos = y + y_offset_image + \
                feature[0, 1] - int(h_sprite*factor)  # right on top
    ...
    while run_event.is_set():
        ...
        for face in faces:
            ...
            if SPRITES[2]:
                ...
                apply_sprite(
                    image, IMAGES[2][ACTIVE_IMAGES[2]], w3, x3-40, y3+30, incl, ontop=False)
                (x3, y3, w3, h3) = get_face_boundbox(shape, 8)  # nose
                apply_sprite(
                    image, IMAGES[2][ACTIVE_IMAGES[2]], w3, x3+40, y3+75, incl)
...
#                apply_sprite(image, IMAGES[5][ACTIVE_IMAGES[5]],w,x,y, incl, ontop = True)
            ...
            if SPRITES[4]:
                ...
                apply_sprite(
                    image, IMAGES[4][ACTIVE_IMAGES[4]], w1+350, x1-230, y1+100, incl,  ontop=False)
